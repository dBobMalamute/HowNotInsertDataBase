...
Item
{
    ...
    Item
    {
        ...
        function oneAction()
        {
            ...
            else if(searchIndex < root.numPeople - 1 && (!found))
            {
                searchIndex++;
    ...
    SequentialAnimation
    {
        ...
        PauseAnimation{duration: 20 * idea.animationSpeed;}
        ScriptAction
        {
            script: if(botBrain.searchIndex > -1)
                    {
                        if(idea.getHeight(botBrain.searchIndex) > insertPersonHeight)
                            insertionLocationMarkers.setYes(botBrain.searchIndex);
                        else
                            insertionLocationMarkers.setNo(botBrain.searchIndex, false);
                    }
        }
        PauseAnimation{duration: 1 * idea.animationSpeed;}
        ScriptAction{script: botBrain.nextAction();}
    ...
    SequentialAnimation
    {
        ...
        ScriptAction
        {
            script: idea.insertionAnimationCompleted();
        }
    }
    ...
    Item
    {
        id: botBox; anchors.fill: track; z: 10
        Image{ id: bot; source: "qrc:/Images/OrangeBot.png";}
    }
